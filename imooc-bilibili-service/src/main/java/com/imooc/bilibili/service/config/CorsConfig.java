...
public class CorsConfig implements Filter {
    ...
    private final String[] allowedDomain = {"http://localhost:8086", "http://39.107.54.180"};
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (allowedOrigins.contains(origin)) {
            ...
            httpResponse.setHeader("Access-Control-Allow-Headers", "Origin, No-Cache, X-Requested-With, " +
                    ...
                    "userId, token, ut");//表明服务器支持的所有头信息字段
            httpResponse.setHeader("Access-Control-Allow-Credentials", "true"); //如果要把Cookie发到服务器，需要指定Access-Control
            // -Allow-Credentials字段为true;
